...
async function createRegistration({ cohortId, track, firstName, lastName, email, phone, company, sector, timeslot, disciplines, enrolledSpecialties, userId }) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      cohort_id:      cohortId,
      track,
      user_id:        userId || null,   // links the registration to its founder login account
      ...
      enrolled_specialties: (Array.isArray(enrolledSpecialties) && enrolledSpecialties.length) ? enrolledSpecialties : null,
      full_fee:       t.fullFee,
...
// ── Founder login account (server-side, robust) ───────────────────────────────
// Creates the founder's Supabase Auth account with the service role, AUTO-CONFIRMED
// (email_confirm:true) so they can sign in immediately — no email-confirmation step,
// which the built-in SMTP rate-limits and which was silently failing client-side.
// Idempotent: if the email already has an account, link to it (never overwrite the
// existing password). Never throws (registration/payment must proceed regardless) and
// never logs the password. Returns { userId, created }.
async function ensureFounderAccount({ email, password, firstName, lastName, phone, company, sector }) {
  const e = (email || '').toLowerCase().trim();
  if (!e || !password) return { userId: null, created: false };
  ...
  try {
    const { data, error } = await supabase.auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { first_name: firstName, last_name: lastName, phone, company, sector },
    });
    ...
    // Email already has an account → link to it; do NOT reset their password.
    const msg = (error && error.message ? error.message : '').toLowerCase();
    const exists = error && (error.status === 422 || msg.includes('already') ||
                             msg.includes('registered') || msg.includes('exist'));
    if (exists) {
      const { data: uid } = await supabase.rpc('auth_user_id_by_email', { p_email: e });
      return { userId: uid || null, created: false };
    }

    console.error('[Account] createUser failed (non-fatal):', error && error.message);
    return { userId: null, created: false };
  } catch (err) {
    console.error('[Account] ensureFounderAccount threw (non-fatal):', err.message);
    return { userId: null, created: false };
  }
}
...
// ── Balance-delinquency grace pathway (roll-forward or refund) ───────────────
// Next open cohort by DATE (not id) — robust to non-sequential ids.
async function getNextOpenCohortByDate(currentCohort) {
  const { data, error } = await supabase
    .from('cohorts')
    .select('*')
    .eq('status', 'open')
    .gt('start_date', currentCohort.start_date)
...
// balance-due timestamp for a cohort = (start 10:00 EAT) − 48h
function balanceDueFor(startDate) {
  if (!startDate) return null;
  const start = new Date(startDate + 'T10:00:00+03:00');
  return new Date(start.getTime() - 48 * 3600000).toISOString();
}

// Founder missed the balance deadline: free their seat + open a decision window.
// Idempotent via the `balance_grace_token IS NULL` guard — safe to re-run.
async function enterBalanceGrace(reg, graceDays = 7) {
  const token    = crypto.randomBytes(24).toString('hex');
  const deadline = new Date(Date.now() + graceDays * 86400000).toISOString();
  const { data, error } = await supabase
    .from('registrations')
    .update({
      balance_grace_token:    token,
      balance_grace_deadline: deadline,
      seat_released:          true,
      updated_at:             new Date().toISOString(),
    })
    .eq('id', reg.id)
    .is('balance_grace_token', null)
    .eq('balance_paid', false)
    .eq('forfeited', false)
    .select('*')
    .maybeSingle();
  if (error) throw error;
  if (!data) return null; // already in grace / handled concurrently
  const field = TRACK_FIELD[reg.track] || 'group_taken';
  await supabase.rpc('decrement_cohort_count', { p_cohort_id: reg.cohort_id, p_field: field });
  return { token, deadline };
}

async function getRegByGraceToken(token) {
  if (!token) return null;
  const { data, error } = await supabase
    .from('registrations')
    .select('*, cohort:cohorts!registrations_cohort_id_fkey(*)')
    .eq('balance_grace_token', token)
    .maybeSingle();
  if (error) throw error;
  return data || null;
}

// Roll everything paid forward into the next open cohort (seat already freed).
// Returns true only if THIS call applied it (guards the founder-click vs lapse-cron
// race with `.is('balance_choice', null)` so the seat counter can't double-count).
async function rollRegistrationForward(reg, nextCohort) {
  const field = TRACK_FIELD[reg.track] || 'group_taken';
  const { data, error } = await supabase
    .from('registrations')
    .update({
      cohort_id:         nextCohort.id,
      moved_to_cohort:   nextCohort.id,
      balance_due_at:    balanceDueFor(nextCohort.start_date),
      balance_choice:    'roll',
      balance_choice_at: new Date().toISOString(),
      seat_released:     false,
      updated_at:        new Date().toISOString(),
    })
    .eq('id', reg.id)
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) return false;                 // already resolved by a concurrent path
  await supabase.rpc('increment_cohort_count', { p_cohort_id: nextCohort.id, p_field: field });
  return true;
}

// Refund what they paid ABOVE the non-refundable deposit. Flags an ops task —
// the actual money movement is issued by finance/ops via the payment provider.
// Returns the refund amount if applied, or null if already resolved (race guard).
async function requestRegistrationRefund(reg) {
  ...
  const { data, error } = await supabase
    .from('registrations')
    .update({
      balance_choice:    'refund',
      balance_choice_at: new Date().toISOString(),
      refund_status:     refund > 0 ? 'requested' : 'cancelled',
      refund_amount:     refund,
      forfeited:         true, // out of the cohort; deposit retained per policy
      updated_at:        new Date().toISOString(),
    })
    .eq('id', reg.id)
    .is('balance_choice', null)
    ...
    .maybeSingle();
  if (error) throw error;
  if (!data) return null;                  // already resolved by a concurrent path
  return refund;
}

module.exports = {
  supabase,
  getNextOpenCohortByDate,
  balanceDueFor,
  enterBalanceGrace,
  getRegByGraceToken,
  rollRegistrationForward,
  requestRegistrationRefund,
  TRACKS,
  getTrackPricing,
  daysUntil,
  getOpenCohorts,
