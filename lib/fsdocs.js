...
function billTo(doc, y, inv) {
  ...
  // The customer's registered address and TIN belong on a tax document — a
  // Ugandan business needs both to claim the expense.
  if (inv.bill_to_address) {
    String(inv.bill_to_address).split(/\s*\n\s*|\s*,\s*(?=\d|[A-Z])/).filter(Boolean)
      .forEach(part => { line(doc, part, M, y, { size: 9 }); y += 11; });
  }
  if (inv.bill_to_tin) { line(doc, `TIN ${inv.bill_to_tin}`, M, y, { size: 8.5, color: MUTED }); y += 12; }
