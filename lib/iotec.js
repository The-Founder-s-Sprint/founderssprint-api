...
/**
 * Sanitise a payer/payee note before sending to ioTec.
 * ioTec support flagged that special characters in the payment reason — notably an
 * apostrophe (') or a double-hyphen (--) — break the Airtel collection path (MTN
 * tolerates them, Airtel does not, which is why identical requests worked on MTN and
 * silently failed on Airtel). We strip apostrophes/quotes, convert every dash variant
 * (— – --) to a space, and remove anything outside a conservative ASCII set. Keeps the
 * note readable ("Founder's Sprint deposit — upcoming cohort" → "Founders Sprint deposit upcoming cohort").
 */
function sanitiseNote(s) {
  return String(s || '')
    .replace(/[‘’'"`“”]/g, '') // apostrophes / smart + straight quotes
    .replace(/[—–]/g, ' ')               // em/en dashes → space
    .replace(/-{2,}/g, ' ')                        // double-hyphen (SQL-comment-like) → space
    .replace(/[^A-Za-z0-9 .,#\/]/g, ' ')           // drop any other special chars
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 100);
}
...
/**
 * Request a CARD collection (Visa/MasterCard), processed by ioTec via PegPay.
 * Unlike Mobile Money there is NO STK push: ioTec returns a hosted-page URL
 * (`cardRedirectUrl`) that we must redirect the customer's browser to. The
 * customer enters their card on PegPay; on completion PegPay returns them to
 * our `redirectUrl` and ioTec fires the SAME callback (/api/iotec/webhook),
 * so crediting reuses the existing webhook + reconcile path unchanged.
 * @param {object} p
 * @param {string} p.email        customer email — the `payer` for card collections
 * @param {string} [p.name]       customer name (populates the PegPay form)
 * @param {number} p.amount       UGX integer, must be >= 500
 * @param {string} p.externalId   our reference (e.g. "FS-DEPOSIT-123") — same scheme as MoMo
 * @param {string} [p.note]       short payer note (<=100 chars)
 * @param {string} p.redirectUrl  absolute https URL PegPay returns the browser to
 * @returns {{ id:string, status:string, internal:string, cardRedirectUrl:string|null, statusMessage:string, raw:object }}
 */
async function requestCardCollection({ email, name, amount, externalId, note, redirectUrl }) {
  const token    = await getAccessToken();
  const walletId = process.env.IOTEC_WALLET_ID;
  if (!walletId) throw new Error('IOTEC_WALLET_ID not configured. Set it to your ioTec Pay collection wallet id.');
  if (!email)    throw new Error('Card collection requires the customer email (payer).');

  const response = await fetch(`${BASE_URL}/collections/collect/card`, {
    method:  'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({
      category:  'Card',
      ...
      walletId,
      externalId,
      payer:     String(email).trim(),          // for Card, `payer` is the customer's EMAIL
      payerName: (name || '').slice(0, 150),
      payerNote: sanitiseNote(note),
      amount,
      redirectUrl,                              // PegPay returns the browser here after payment
    }),
  });

  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw new Error(`ioTec card collection request failed (${response.status}): ${JSON.stringify(data)}`);
  }

  const { internal } = mapStatus(data.status);
  return {
    id:              data.id,                                       // ioTec transaction id (uuid)
    status:          data.status,
    internal,
    cardRedirectUrl: data.cardRedirectUrl || data.redirectUrl || null,  // PegPay hosted page to send the payer to
    statusMessage:   data.statusMessage || data.statusCode || 'Redirect to complete card payment',
    raw:             data,
  };
}
