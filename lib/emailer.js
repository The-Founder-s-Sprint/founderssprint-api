...
const ADMIN_EMAIL   = process.env.ADMIN_EMAIL   || 'teddy@tmsruge.com';
...
async function sendConfirmation(reg, cohort) {
  const isVip          = reg.track === 'vip';
  const templateName   = isVip ? 'confirmation_vip' : 'confirmation';
  const balanceDueDate = fmtBalanceDueDate(cohort.start_date);
  const icsAttachment  = generateICS(cohort, reg);
  ...
  return sendWithTemplate(templateName, {
    ...
    ADMIN_EMAIL,
    TRACK:            reg.track === 'oneOnOne' ? '1-on-1 Coaching' : 'Group',
    INTAKE_FORM_URL:  process.env.VIP_INTAKE_FORM_URL || 'https://tmsruge.com/vip-intake',
  }, {
    to:             reg.email,
    subject:        isVip
      ? `Your VIP spot is reserved · ${cohort.name} · The Founder's Sprint`
      : `You're registered · ${cohort.name} · The Founder's Sprint`,
...
async function sendNewRegistrationAdmin(reg, cohort) {
  const trackLabel = { group: 'Group', oneOnOne: '1-on-1', vip: 'VIP All-Access' }[reg.track] || reg.track;
...
<html><body style="font-family:Arial,sans-serif;max-width:600px;margin:40px auto;color:#222;line-height:1.6">
  ...
  <p style="color:#666;margin-top:0">${cohort.name} · The Founder's Sprint</p>
...
async function sendCoachApproval(application) {
