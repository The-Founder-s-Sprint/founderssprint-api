...
function fsEmailShell({ accent = '#C8531F', eyebrow = '', eyebrowBg = 'rgba(200,83,31,0.15)', eyebrowColor = '#9A3E16', headline = '', bodyHtml = '' }) {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#D8CFBE;"><tr><td align="center" style="padding:30px 10px;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:600px;background:#EFE7D8;">
...
async function sendReservationConfirmation(reg, cohort) {
  ...
  const cohortMeta = cohort ? `<div style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;margin-top:6px;">${cohort.name} · ${cohort.dates || `${cohort.start_date} – ${cohort.end_date}`}</div>` : '';
  const bodyHtml = `
    <p style="margin:0 0 16px">Hi ${reg.first_name},</p>
    <p style="margin:0 0 22px">Thanks for reserving your <strong style="color:#1A1A1A">${trackLabel}</strong> place — it's now saved to your account. <strong style="color:#1A1A1A">You haven't been charged anything today.</strong></p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 24px">
      ...
      <tr><td style="padding:16px 22px;">
        ...
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:21px;color:#1A1A1A;">${trackLabel} · Booking #${reg.id}</div>
        ${cohortMeta}
      </td></tr>
    </table>
    <p style="margin:0 0 18px">We're putting the final pieces in place on secure mobile-money payments. The moment booking opens, we'll email you to pay your <strong style="color:#1A1A1A">10% deposit (${fmt(reg.deposit_amount)})</strong> and lock in your spot.</p>
    <p style="margin:0 0 24px">In the meantime, explore the platform and log in to your dashboard any time. Have thoughts? Just reply to this email — your feedback shapes what we ship next.</p>
    <p style="margin:0;font-family:'Cormorant Garamond',Georgia,serif;font-style:italic;font-size:18px;color:#1A1A1A;">— Teddy &amp; the Founder's Sprint team</p>`;
  const html = fsEmailShell({
    accent: '#C8531F',
    eyebrow: 'Booking opens soon',
    eyebrowBg: 'rgba(200,83,31,0.15)', eyebrowColor: '#9A3E16',
    headline: 'Your place is reserved.',
    bodyHtml,
  });
  return sendEmail({
    to:             reg.email,
...
async function sendNewRegistrationAdmin(reg, cohort, isPreview = false) {
  ...
  const tableRows = rows.map((r, i) =>
    `<tr style="background:${i % 2 ? '#EFE7D8' : '#E6DCC7'}">
