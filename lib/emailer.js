...
const { logEmail, supabase } = require('./db');
const { renderDocument, documentFilename } = require('./fsdocs');
...
const FINANCE_EMAIL = process.env.FINANCE_EMAIL || 'finance@founderssprint.co';
// Beta express-session mobile-money details (Teddy's personal MoMo line).
const EXPRESS_MM      = process.env.EXPRESS_MM_NUMBER || process.env.AIRTEL_NUMBER || '07XX XXX XXX';
const EXPRESS_MM_NAME = process.env.EXPRESS_MM_NAME   || 'Teddy Ruge';
...
// ── Core send ─────────────────────────────────────────────────────────────────
async function sendEmail({ to, subject, html, registrationId = null, template, attachments = [], bcc = null }) {
  try {
    const payload = {
    ...
    // BCC, not CC: finance needs the books, the founder does not need to see
    // who else is copied on their receipt. Silently dropped if it would just
    // mail finance a copy of finance's own mail.
    if (bcc) {
      const list = (Array.isArray(bcc) ? bcc : [bcc])
        .map(e => String(e || '').trim().toLowerCase())
        .filter(e => e && e !== String(to || '').trim().toLowerCase());
      if (list.length) payload.bcc = list;
    }
...
/**
 * Slug → display name from taxonomy_specialties, the single source of truth for
 * the 49 L3 names. Cached for the life of the process: the taxonomy changes on
 * a migration, not between two emails.
 *
 * Never throws — a label lookup must not be able to stop a registration email.
 */
let _specCache = null;
async function specialtyLabels(slugs) {
  if (!Array.isArray(slugs) || !slugs.length) return {};
  if (_specCache) return _specCache;
  try {
    const { data, error } = await supabase.from('taxonomy_specialties').select('slug, name');
    if (error || !data) return {};
    _specCache = Object.fromEntries(data.map(r => [r.slug, r.name]));
    return _specCache;
  } catch (e) {
    console.error('[emailer] specialty label lookup failed:', e.message);
    return {};
  }
}
...
async function sendNewRegistrationAdmin(reg, cohort, isPreview = false) {
  ...
  // What they actually booked. This was written for 1-on-1s, where the list is
  // one or three items — handed a cohort it printed all 49 specialty names and
  // buried every other field in the email. A cohort seat IS the whole catalogue,
  // so describe it; only enumerate when the list is genuinely a choice.
  const specs = Array.isArray(reg.enrolled_specialties) ? reg.enrolled_specialties.filter(Boolean) : [];
  // Real names live in taxonomy_specialties ("CAC & LTV", "SEO & Discoverability").
  // Title-casing the slug produced "Cac And Ltv" — inventing a label for data we
  // already hold. Look it up; prettify only what the table doesn't know about.
  const specNames = await specialtyLabels(specs);
  const pretty = s => specNames[s]
    || String(s).split('-').map(w => w ? w.charAt(0).toUpperCase() + w.slice(1) : w).join(' ');
  let specLabel = null, specLine = null;
  if (reg.track === 'cohort') {
    specLabel = 'Covers';
    specLine  = `All five disciplines — Marketing &amp; Branding, Financial Modelling, Investment Readiness, `
              + `Strategy &amp; Team Building, Product Development &amp; Pricing — `
              + `${specs.length || 49} specialty sessions across the five weeks.`;
  } else if (specs.length) {
    specLabel = specs.length > 1 ? 'Sessions' : 'Session';
    specLine  = specs.length > 8
      ? specs.slice(0, 8).map(pretty).join(', ') + ` … and ${specs.length - 8} more`
      : specs.map(pretty).join(', ');
  }
  ...
  if (specLine) rows.push([specLabel, specLine]);
  rows.push(['Full fee',    fmt(reg.full_fee)]);
  rows.push(['Deposit due', `<strong style="color:#1A1A1A">${fmt(reg.deposit_amount)}</strong>`]);
  rows.push(['Booking ID',  `#${reg.id}`]);
...
async function sendRefundOpsAlert(reg, cohort, refundAmount) {
  ...
  return sendEmail({ to: FINANCE_EMAIL, subject: `Refund to issue · ${fmt(refundAmount)} · ${reg.first_name} ${reg.last_name}`, html, template: 'refund_ops_alert' });
}
...
// Finance record copy — sent to FINANCE_EMAIL whenever a payment is confirmed, for
// record-keeping until QuickBooks Online sync is wired (needs the company bank account first).
async function sendFinancePaymentRecord(reg, cohort, paymentType, meta = {}) {
  const isDeposit = paymentType === 'deposit';
  const amount    = isDeposit ? reg.deposit_amount : reg.balance_amount;
  const when      = new Date().toLocaleString('en-GB', { timeZone: 'Africa/Kampala', dateStyle: 'medium', timeStyle: 'short' });
  const row = (l, v) => `<tr><td style="padding:6px 0;border-bottom:1px solid rgba(26,26,26,0.08);font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;">${l}</td><td align="right" style="padding:6px 0;border-bottom:1px solid rgba(26,26,26,0.08);font-family:'Inter',Arial,sans-serif;font-size:13px;font-weight:600;color:#1A1A1A;">${v}</td></tr>`;
  const payer = `${reg.first_name || ''} ${reg.last_name || ''}`.trim() || '—';
  const bodyHtml = `
    <p style="margin:0 0 18px;">A ${isDeposit ? 'deposit' : 'balance'} payment was confirmed by ioTec — recorded for accounting.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 18px 0;margin:0 0 18px;">
      <tr><td style="padding:16px 20px 4px;text-align:center;"><span style="font-family:'Cormorant Garamond',Georgia,serif;font-size:34px;color:#1A1A1A;">${fmt(amount)}</span></td></tr>
      <tr><td style="padding:0 20px 14px;text-align:center;"><span style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8a6a1f;">${isDeposit ? 'Deposit (10%)' : 'Balance — fully paid'}</span></td></tr>
    </table>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
      ${row('Payer', payer)}
      ${row('Email', reg.email || '—')}
      ${reg.company ? row('Company', reg.company) : ''}
      ${row('Product / track', reg.track || '—')}
      ${row('Payment type', isDeposit ? 'Deposit (10%)' : 'Balance')}
      ${row('Method', meta.method || 'mobile money')}
      ${meta.reference ? row('ioTec reference', meta.reference) : ''}
      ${cohort && cohort.name ? row('Cohort', cohort.name) : ''}
      ${row('Registration', 'FS-' + reg.id)}
      ${meta.receipt ? row('Receipt', meta.receipt) : ''}
      ${row('Confirmed', when + ' EAT')}
    </table>
    <p style="margin:18px 0 0;font-family:'Inter',Arial,sans-serif;font-size:12px;line-height:1.6;color:#5A564F;">Automated record for finance. The numbered receipt${meta.receipt ? ' (' + meta.receipt + ')' : ''} is the document of record — mirror it into the ledger by that number rather than issuing a new one.</p>`;
  const html = fsEmailShell({ accent: '#C9923A', eyebrow: 'Payment record', eyebrowColor: '#8a6a1f', eyebrowBg: 'rgba(201,146,58,0.18)', headline: 'Payment received', bodyHtml });
  return sendEmail({
    to:             FINANCE_EMAIL,
    subject:        `Payment · ${fmt(amount)} · ${payer} · ${isDeposit ? 'deposit' : 'balance'}`
                    + (meta.receipt ? ` · ${meta.receipt}` : ''),
    html,
    template:       'finance_payment_record',
    registrationId: reg.id,
  });
}

/**
 * Find the numbered receipt for one payment on one registration, and the parent
 * invoice it settles against.
 *
 * The only key used is `registration_id` — taken from the registration the
 * caller already loaded, never from anything a client supplied. There is no
 * document id on the wire, so there is no document to enumerate: a caller can
 * only ever reach the documents belonging to the registration they hold.
 *
 * Returns { receipt, invoice } with nulls when nothing is on file. Most
 * registrations predate the invoicing system and will legitimately have none.
 */
async function receiptForPayment(registrationId, paymentType, recipientEmail = null) {
  const empty = { receipt: null, invoice: null };
  if (!registrationId) return empty;
  try {
    const { data, error } = await supabase
      .from('invoices')
      .select('number, doc_type, currency, bill_to_name, bill_to_email, bill_to_company, ' +
              'bill_to_pending, bill_to_tin, bill_to_address, ' +
              'subtotal, vat_amount, total, amount_paid, line_items, payments, issued_at, ' +
              'paid_on, method, reference_note, status')
      .eq('registration_id', registrationId)
      .in('doc_type', ['invoice', 'receipt'])
      .order('number', { ascending: true });
    if (error || !data) return empty;

    const invoice = data.find(d => d.doc_type === 'invoice') || null;
    // A registration can hold several receipts; take the latest one issued for
    // this payment type, so a re-issued receipt supersedes the original.
    const receipt = data
      .filter(d => d.doc_type === 'receipt' && ((d.payments || [])[0] || {}).type === paymentType)
      .pop() || null;

    // An incomplete bill-to is not a tax-valid document. The row is still
    // numbered — numbering must stay gapless — but sending "Attn: Pending" to a
    // paying customer is worse than sending nothing, and worse still if they
    // file it. Finance completes the bill-to, then re-sends.
    if (receipt && (receipt.bill_to_pending || (invoice && invoice.bill_to_pending))) {
      console.warn('[emailer] receipt ' + receipt.number +
        ' has an incomplete bill-to — withheld until finance completes it');
      return { receipt: null, invoice: null };
    }

    // A registration can be paid for by someone other than the founder sitting in
    // the seat — a corporate sponsor, a hub, a parent company. In that case the
    // billing document belongs to the payer, not the attendee, and mailing it to
    // the attendee would hand them a third party's billing details. Attach only
    // when the document is addressed to the person we are writing to.
    if (receipt && recipientEmail && receipt.bill_to_email &&
        receipt.bill_to_email.trim().toLowerCase() !== String(recipientEmail).trim().toLowerCase()) {
      console.warn('[emailer] receipt ' + receipt.number + ' is billed to a third party — not attaching');
      return { receipt: null, invoice: null };
    }

    return { receipt, invoice };
  } catch (e) {
    // A missing document must never block the payment confirmation itself.
    console.error('[emailer] receipt lookup failed:', e.message);
    return empty;
  }
}
...
async function sendPaymentConfirmation(reg, cohort, paymentType) {
  ...
  // The numbered receipt, if one exists. Generated fresh from the ledger row on
  // every send, so a corrected bill-to reaches the founder without anyone having
  // to regenerate and re-upload a file.
  const { receipt, invoice } = await receiptForPayment(reg.id, paymentType, reg.email);
  let attachments = [];
  if (receipt) {
    try {
      attachments = [{
        filename:     documentFilename(receipt),
        content:      await renderDocument(receipt, invoice),
        content_type: 'application/pdf',
      }];
    } catch (e) {
      console.error('[emailer] receipt render failed for ' + receipt.number + ':', e.message);
      attachments = [];
    }
  }
  ...
  const bodyHtml = `
    ...
    ${receipt ? `<p style="margin:0 0 22px;">Your receipt <strong style="color:#1A1A1A">${receipt.number}</strong> is attached as a PDF — keep it for your records${invoice ? ` (it settles against invoice <strong style="color:#1A1A1A">${invoice.number}</strong>)` : ''}.</p>` : ''}
    ${emailBtn('Go to your dashboard', BASE + '/login-founder.html')}
  `;
  const html = fsEmailShell({
    accent:       '#8AAB5C',
  ...
  return sendEmail({
    ...
    registrationId: reg.id,
    attachments,
    // Finance gets the numbered receipt PDF itself, not just a summary — it is
    // the document of record, and the books should be buildable from the inbox
    // without anyone re-exporting anything. Withheld when the receipt was
    // withheld (third-party payer / bill-to pending) so finance never receives a
    // document the customer hasn't.
    bcc:            attachments.length ? FINANCE_EMAIL : null,
  });
}
...
// ── Beta express 1:1 consulting sessions (manual mobile-money) ───────────────
function fmtSlotEAT(iso) {
  const d = new Date(iso);
  const date = d.toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Africa/Nairobi' });
  const time = d.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'Africa/Nairobi' });
  return `${date} · ${time} EAT`;
}

// Founder — session held, here's how to pay by mobile money.
async function sendExpressBookingRequest(b) {
  const BASE = 'https://founderssprint.co';
  const when = fmtSlotEAT(b.slot_start);
  const kv = (k, v) => `<tr><td style="padding:6px 0;font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#5A564F;">${k}</td><td align="right" style="padding:6px 0;font-family:'Inter',Arial,sans-serif;font-size:15px;color:#1A1A1A;">${v}</td></tr>`;
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${b.first_name},</p>
    <p style="margin:0 0 20px;">Your <strong style="color:#1A1A1A">${b.duration_min}-minute express session</strong> with Teddy is <strong style="color:#1A1A1A">held</strong> for:</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#C8531F;">&nbsp;</td></tr>
      <tr><td style="padding:16px 22px;">
        <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#5A564F;margin-bottom:6px;">Your session</div>
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:21px;color:#1A1A1A;line-height:1.3;">${when}</div>
        <div style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;margin-top:4px;">Booking ref ${b.payment_ref}</div>
      </td></tr>
    </table>
    <p style="margin:0 0 14px;">To confirm your slot, send <strong style="color:#1A1A1A">${fmt(b.amount)}</strong> by mobile money:</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border:1px solid rgba(200,83,31,0.35);margin:0 0 12px;">
      <tr><td style="padding:16px 20px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        ${kv('MTN / Airtel number', `<strong>${EXPRESS_MM}</strong>`)}
        ${kv('Registered to', EXPRESS_MM_NAME)}
        ${kv('Amount', fmt(b.amount))}
        ${kv('Reference', `<strong>${b.payment_ref}</strong>`)}
      </table></td></tr>
    </table>
    <p style="margin:0 0 22px;font-size:13.5px;color:#5A564F;">Add the reference to the payment message if your app allows it. Slots are first-come — paying sooner locks yours in.</p>
    <p style="margin:0 0 14px;">The moment your payment lands, I'll confirm and email you the Google Meet link. You've also got a dashboard now:</p>
    ${emailBtn('Open your dashboard', BASE + '/login-founder.html')}
    <p style="margin:20px 0 0;">See you soon,<br>— Teddy</p>
  `;
  const html = fsEmailShell({ eyebrow: 'Session held · pay to confirm', headline: `Your express session is held, ${b.first_name}.`, bodyHtml });
  return sendEmail({ to: b.email, subject: `Your express session is held · pay to confirm · The Founder's Sprint`, html, template: 'express_request' });
}

// Teddy — a new express booking is awaiting payment confirmation.
async function sendExpressBookingAdmin(b) {
  const when = fmtSlotEAT(b.slot_start);
  const kv = (k, v) => `<tr><td style="padding:6px 0;font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:1.5px;text-transform:uppercase;color:#5A564F;">${k}</td><td align="right" style="padding:6px 0;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#1A1A1A;">${v}</td></tr>`;
  const bodyHtml = `
    <p style="margin:0 0 18px;">A founder booked an express session and is paying by mobile money. Confirm the payment in the Command Centre, then release the Meet link.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 18px 0;margin:0 0 18px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#C9923A;">&nbsp;</td></tr>
      <tr><td style="padding:14px 20px;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0">
        ${kv('Founder', `${b.first_name} ${b.last_name || ''}`)}
        ${kv('Email', b.email)}
        ${kv('Phone', b.phone || '—')}
        ${kv('Session', when)}
        ${kv('Amount', fmt(b.amount))}
        ${kv('Reference', b.payment_ref)}
      </table></td></tr>
    </table>
    <p style="margin:0;">${emailBtn('Open the Command Centre', 'https://founderssprint.co/admin-portals.html', '#2A2826')}</p>
  `;
  const html = fsEmailShell({ accent: '#C9923A', eyebrow: 'New express booking', eyebrowColor: '#8A6D1F', eyebrowBg: 'rgba(201,146,58,0.18)', headline: 'An express session needs confirming.', bodyHtml });
  return sendEmail({ to: ADMIN_EMAIL, subject: `New express booking · ${b.first_name} ${b.last_name || ''} · ${fmt(b.amount)}`, html, template: 'express_admin' });
}

// Founder — payment confirmed, here's your Meet link.
async function sendExpressConfirmed(b) {
  const when = fmtSlotEAT(b.slot_start);
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${b.first_name},</p>
    <p style="margin:0 0 22px;">Payment received — your express session with Teddy is <strong style="color:#1A1A1A">confirmed</strong>.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#8AAB5C;">&nbsp;</td></tr>
      <tr><td style="padding:16px 22px;">
        <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#3D4A2E;margin-bottom:6px;">Confirmed</div>
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:21px;color:#1A1A1A;line-height:1.3;">${when}</div>
        <div style="font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;margin-top:4px;">${b.duration_min} minutes · 1:1 with Teddy</div>
      </td></tr>
    </table>
    ${b.meet_link
      ? emailBtn('Join on Google Meet', b.meet_link, '#3D4A2E') + `<p style="margin:12px 0 0;font-size:13px;color:#5A564F;">Or paste this link: ${b.meet_link}</p>`
      : `<p style="margin:0;">Your Google Meet link will follow shortly.</p>`}
    <p style="margin:22px 0 0;">Come with your sharpest question — we'll make the hour count.<br>— Teddy</p>
  `;
  const html = fsEmailShell({ accent: '#8AAB5C', eyebrow: 'Session confirmed', eyebrowColor: '#3D4A2E', eyebrowBg: 'rgba(138,171,92,0.18)', headline: "You're confirmed.", bodyHtml });
  return sendEmail({ to: b.email, subject: `Confirmed · your express session · ${when} · The Founder's Sprint`, html, template: 'express_confirmed' });
}
...
async function sendDirectoryReminder(provider, type, renewUrlArg) {
  ...
  // Payment-gated renew page (tokenised) when supplied by the caller; safe fallback otherwise.
  const renewUrl = renewUrlArg || `https://founderssprint.co/apply-directory.html`;
...
async function sendDirectoryExpired(provider, renewUrlArg) {
  const tierLabel = TIER_LABELS[provider.tier] || provider.tier;
  const tierPrice = TIER_PRICES[provider.tier] || '';
  const renewUrl = renewUrlArg || `https://founderssprint.co/apply-directory.html`;
...
// Secure "here's your renewal link" email — sent by the admin console or the lifecycle
// reminders. The link carries the opaque token that authorises the renew page.
async function sendDirectoryRenewLink(provider, renewUrl) {
  const tierLabel = TIER_LABELS[provider.tier] || provider.tier;
  const tierPrice = TIER_PRICES[provider.tier] || '';
  const body = `
    <p>Hi ${provider.contact_name || 'there'},</p>
    <p>Here's your secure link to renew the <strong>${tierLabel}</strong> listing for <strong>${provider.company_name}</strong> on The Founder's Sprint directory:</p>
    <div style="margin:24px 0">
      <a href="${renewUrl}" style="display:inline-block;background:#C8531F;color:#EFE7D8;font-family:Arial,sans-serif;font-size:13px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;padding:14px 28px;text-decoration:none">Renew &amp; pay</a>
    </div>
    <p style="font-size:14px;color:#5A564F">Current renewal rate: ${tierPrice}. You'll see the exact amount — and the audience you're reaching — before you pay.</p>
    <p style="font-size:13px;color:#5A564F">This link is unique to your listing, so please don't share it.</p>`;
  const html = directoryEmailShell('Renew your listing', body);
  return sendEmail({
    to:       provider.email,
    subject:  `Renew your listing · ${provider.company_name} · The Founder's Sprint Directory`,
    html,
    template: 'directory_renew_link',
  });
}

// ── Monthly founders' digest (platform summary + per-coach section) ───────────
function digestSection(title, rows) {
  const body = rows.map(r => `<tr>
    <td style="padding:7px 0;border-bottom:1px solid rgba(26,26,26,0.08);font-family:'Inter',Arial,sans-serif;font-size:14px;color:#2A2826;">${r[0]}</td>
    <td align="right" style="padding:7px 0;border-bottom:1px solid rgba(26,26,26,0.08);font-family:'Cormorant Garamond',Georgia,serif;font-size:19px;font-weight:600;color:#1A1A1A;white-space:nowrap;">${r[1]}</td>
  </tr>`).join('');
  return `<div style="margin:0 0 22px;">
    <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#9A3E16;margin:0 0 4px;">${title}</div>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0">${body}</table>
  </div>`;
}

async function sendCoachMonthlyDigest(coach, digest, monthLabel) {
  const BASE = 'https://founderssprint.co';
  const p   = digest.platform || {};
  const dir = p.directory || {}, bc = dir.by_channel || {}, rev = p.revenue || {}, co = p.coaching || {};
  const c   = coach || {};
  const n   = v => Number(v || 0).toLocaleString('en-UG');

  const platformBlocks =
    digestSection('Growth', [
      ['New founder accounts', n(p.new_accounts)],
      ['New course registrations', n(p.new_registrations)],
      ['Deposits paid', n(p.deposits_paid)],
    ]) +
    digestSection('Traffic', [['Website page views', n(p.pageviews)]]) +
    digestSection('Directory', [
      ['Contact click-outs', n(dir.clickouts)],
      ['Email / Website / WhatsApp / Call', `${n(bc.email)} / ${n(bc.website)} / ${n(bc.whatsapp)} / ${n(bc.call)}`],
      ['Active listings', n(dir.active_listings)],
      ['Paid renewals', n(dir.renewals_paid)],
    ]) +
    digestSection('Coaching', [
      ['Sessions scheduled', n(co.sessions)],
      ['Sessions completed', n(co.completed)],
    ]) +
    digestSection('Revenue &amp; founder payments', [
      ['Collected this month', fmt(rev.collected)],
      ['Deposits / balances', `${fmt(rev.deposits)} · ${fmt(rev.balances)}`],
      ['Outstanding balances', fmt(rev.outstanding)],
    ]);

  const hasActivity = (Number(c.sessions) || 0) > 0 || (Number(c.gross) || 0) > 0;
  const personal = `<div style="margin:6px 0 24px;padding:18px 20px;background:#E6DCC7;border-radius:0 0 18px 0;">
      <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#3D4A2E;margin:0 0 8px;">Your month, ${c.first_name || 'Coach'}</div>
      ${hasActivity
        ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0">
             <tr><td style="padding:5px 0;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#2A2826;">Sessions delivered</td><td align="right" style="font-family:'Cormorant Garamond',Georgia,serif;font-size:20px;font-weight:600;color:#1A1A1A;">${n(c.sessions)}</td></tr>
             <tr><td style="padding:5px 0;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#2A2826;">Earnings (gross)</td><td align="right" style="font-family:'Cormorant Garamond',Georgia,serif;font-size:20px;font-weight:600;color:#1A1A1A;">${fmt(c.gross)}</td></tr>
             <tr><td style="padding:5px 0;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#2A2826;">Withholding tax (URA)</td><td align="right" style="font-family:'Cormorant Garamond',Georgia,serif;font-size:18px;color:#5A564F;">${fmt(c.wht)}</td></tr>
             <tr><td style="padding:5px 0;font-family:'Inter',Arial,sans-serif;font-size:14px;color:#2A2826;">Cash payable</td><td align="right" style="font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;font-weight:600;color:#3D4A2E;">${fmt(c.cash)}</td></tr>
           </table>`
        : `<div style="font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.6;color:#5A564F;">No sessions logged this month yet — your coaching activity and earnings will appear here as bookings come through.</div>`}
    </div>`;

  const bodyHtml = `
    <p style="margin:0 0 8px;">Hi ${c.first_name || 'there'},</p>
    <p style="margin:0 0 22px;">Here's how The Founder's Sprint moved in <strong style="color:#1A1A1A">${monthLabel}</strong>.</p>
    ${personal}
    <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#1A1A1A;margin:0 0 14px;border-bottom:2px solid #C8531F;padding-bottom:6px;">Platform this month</div>
    ${platformBlocks}
    ${emailBtn('Open the dashboard', BASE + '/login-coach.html')}
    <p style="margin:18px 0 0;font-family:'Inter',Arial,sans-serif;font-size:13px;line-height:1.6;color:#5A564F;">You're receiving this as a founding coach of The Founder's Sprint. Reply any time with what you'd like to see in next month's digest.</p>`;

  const html = fsEmailShell({ eyebrow: 'Monthly Digest · ' + monthLabel, headline: 'The platform, at a glance', bodyHtml });
  return sendEmail({ to: c.email, subject: `Founder's Sprint · ${monthLabel} digest`, html, template: 'coach_monthly_digest' });
}

// ── Assignments ──────────────────────────────────────────────────────────────
// The point of these two is to stop the conversation leaking back to WhatsApp.
// So they carry enough to be useful on a phone lock screen — who, what, when —
// and then send the person to the dashboard to actually answer, because that is
// where the reply stays attached to the work.
const ASG_BASE = 'https://founderssprint.co';
function asgEsc(s) {
  return String(s == null ? '' : s)
    .replace(/[&<>"]/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
}
// Never paste a whole submission into an email: it is the founder's work, the
// dashboard is the record, and long bodies wreck the layout.
function asgQuote(body) {
  if (!body) return '';
  const t = String(body).trim();
  const short = t.length > 400 ? t.slice(0, 400).replace(/\s+\S*$/, '') + '…' : t;
  return `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;margin:0 0 22px;">
      <tr><td style="padding:14px 18px;font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.65;color:#2A2826;white-space:pre-wrap;">${asgEsc(short)}</td></tr></table>`;
}
// A subject is a header, not HTML. Strip anything that could break the line or
// smuggle a second header, and keep it short enough to survive a phone preview.
function asgSubject(s) {
  const t = String(s || '')
    .replace(/[\r\n\t]+/g, ' ')
    .replace(/[\x00-\x1F\x7F]/g, '')
    .trim();
  return t.length > 120 ? t.slice(0, 117) + '…' : (t || 'Assignment');
}
function asgDueLine(dueAt) {
  if (!dueAt) return '';
  const d = new Date(dueAt).toLocaleDateString('en-GB',
    { timeZone: 'Africa/Kampala', weekday: 'long', day: 'numeric', month: 'long' });
  return `<p style="margin:0 0 22px;">Due by <strong style="color:#1A1A1A">${asgEsc(d)}</strong>.</p>`;
}

async function sendAssignmentSet({ to, firstName, coachName, title, brief, dueAt }) {
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
    <p style="margin:0 0 22px;"><strong style="color:#1A1A1A">${asgEsc(coachName)}</strong> has set you a piece of work.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#C8531F;">&nbsp;</td></tr>
      <tr><td style="padding:16px 22px;">
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:22px;color:#1A1A1A;">${asgEsc(title)}</div>
        ${brief ? `<div style="font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.65;color:#2A2826;margin-top:10px;white-space:pre-wrap;">${asgEsc(String(brief).slice(0, 600))}</div>` : ''}
      </td></tr>
    </table>
    ${asgDueLine(dueAt)}
    <p style="margin:0 0 22px;">Answer it on your dashboard rather than by email — your response and any questions stay attached to the assignment, where your coach can see them alongside the work.</p>
    ${emailBtn('Open the assignment', ASG_BASE + '/login-founder.html')}`;
  const html = fsEmailShell({ eyebrow: 'New assignment', headline: 'Your coach has set you work.', bodyHtml });
  return sendEmail({ to, subject: asgSubject(`Assignment · ${title}`), html, template: 'assignment_set' });
}

async function sendAssignmentReply({ to, forRole, firstName, fromName, title, status, body, submission }) {
  const toCoach = forRole === 'coach';
  const verdict = { returned: 'sent back for another go', accepted: 'accepted' }[status];
  const lead = toCoach
    ? (submission
        ? `<strong style="color:#1A1A1A">${asgEsc(fromName)}</strong> has submitted their work.`
        : `<strong style="color:#1A1A1A">${asgEsc(fromName)}</strong> has a question about this assignment.`)
    : (body
        ? `<strong style="color:#1A1A1A">${asgEsc(fromName)}</strong> has replied${verdict ? ` and ${verdict} your submission` : ''}.`
        : `<strong style="color:#1A1A1A">${asgEsc(fromName)}</strong> has ${verdict || 'updated your submission'}.`);
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
    <p style="margin:0 0 22px;">${lead}</p>
    <p style="margin:0 0 10px;font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#5A564F;">${asgEsc(title)}</p>
    ${asgQuote(body || submission)}
    ${emailBtn(toCoach ? 'Open the thread' : 'Open the assignment',
               ASG_BASE + (toCoach ? '/login-coach.html' : '/login-founder.html'))}`;
  const html = fsEmailShell({
    accent:  status === 'accepted' ? '#3D4A2E' : '#C8531F',
    eyebrow: toCoach ? 'From your founder' : 'From your coach',
    headline: toCoach ? 'There’s something waiting for you.' : 'Your coach has come back to you.',
    bodyHtml,
  });
  return sendEmail({ to, subject: asgSubject(`${toCoach ? fromName : 'Your coach'} · ${title}`), html, template: 'assignment_reply' });
}

// One mail per person per run, listing everything — see the cron's note on why
// a per-item digest is self-defeating.
async function sendAssignmentDigest({ to, forRole, firstName, items = [], pendingReviews = 0 }) {
  const toCoach = forRole === 'coach';
  let bodyHtml;
  if (toCoach) {
    bodyHtml = `
      <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
      <p style="margin:0 0 22px;">You have <strong style="color:#1A1A1A">${pendingReviews}</strong> submission${pendingReviews === 1 ? '' : 's'} waiting for a look.</p>
      ${emailBtn('Review them', ASG_BASE + '/login-coach.html')}`;
  } else {
    const list = items.map(i => {
      const d = new Date(i.dueAt).toLocaleDateString('en-GB', { timeZone: 'Africa/Kampala', weekday: 'long', day: 'numeric', month: 'long' });
      return `<tr><td style="padding:9px 0;border-top:1px solid rgba(26,26,26,0.12);font-family:'Inter',Arial,sans-serif;font-size:14px;color:#1A1A1A;">${asgEsc(i.title)}
        <div style="font-family:'Inter',Arial,sans-serif;font-size:12px;color:${i.overdue ? '#9A3E16' : '#5A564F'};margin-top:2px;">${i.overdue ? 'Was due ' : 'Due '}${asgEsc(d)}</div></td></tr>`;
    }).join('');
    bodyHtml = `
      <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
      <p style="margin:0 0 18px;">${items.length === 1 ? 'One piece of work is' : `${items.length} pieces of work are`} coming due.</p>
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 22px;">${list}</table>
      <p style="margin:0 0 22px;">If something's in the way, say so on the thread — your coach would far rather know now than find out on the day.</p>
      ${emailBtn('Open your assignments', ASG_BASE + '/login-founder.html')}`;
  }
  const html = fsEmailShell({
    eyebrow:  toCoach ? 'Waiting on you' : 'Coming due',
    headline: toCoach ? 'Submissions to review.' : 'A quick heads-up.',
    bodyHtml,
  });
  return sendEmail({
    to,
    subject: toCoach ? `${pendingReviews} submission${pendingReviews === 1 ? '' : 's'} to review` : 'Your assignments · coming due',
    html, template: 'assignment_digest',
  });
}

// ── Postponement & withdrawal ────────────────────────────────────────────────
// Someone raising one of these is usually having a bad week. The tone is plain
// and unbureaucratic, the numbers are stated rather than linked to, and nothing
// asks them to chase us.
const ENR_LABEL = { defer: 'postponement', cancel: 'withdrawal' };

async function sendEnrollmentRequestFounder({ to, firstName, kind, quote, cohortName }) {
  const isD = kind === 'defer';
  const money = n => 'UGX ' + Number(n || 0).toLocaleString('en-GB');
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
    <p style="margin:0 0 22px;">We've got your ${ENR_LABEL[kind]} request${cohortName ? ` for ${asgEsc(cohortName)}` : ''}. Nothing more is needed from you — someone on our team will come back to you shortly.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:${isD ? '#C9923A' : '#9A3E16'};">&nbsp;</td></tr>
      <tr><td style="padding:16px 22px;font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.65;color:#2A2826;">
        ${isD
          ? `<strong style="color:#1A1A1A">${money(quote.credit_amount)}</strong> carries forward in full, and you'd start again at Week 1. Nothing is charged for postponing.`
          : `If approved, your refund would be <strong style="color:#1A1A1A">${money(quote.refund_amount)}</strong> — ${money(quote.amount_paid)} paid, less the ${money(quote.deposit_retained)} deposit, less ${money(quote.delivery_charge)} for the ${quote.sessions_delivered} of ${quote.sessions_total} sessions already delivered.`}
      </td></tr>
    </table>
    ${isD ? '' : `<p style="margin:0 0 22px;">If you'd rather keep the door open, tell us and we'll postpone you instead — that costs you nothing and your place is held.</p>`}
    <p style="margin:0 0 22px;">If anything changes before we get back to you, just reply to this email.</p>`;
  const html = fsEmailShell({
    accent: isD ? '#C9923A' : '#9A3E16',
    eyebrow: 'Request received', eyebrowColor: '#9A3E16',
    headline: isD ? 'We’ll hold your place.' : 'We’ve got your request.',
    bodyHtml,
  });
  return sendEmail({ to, subject: asgSubject(`Your ${ENR_LABEL[kind]} request`), html, template: 'enrollment_request_founder' });
}

async function sendEnrollmentRequestStaff({ founderName, founderEmail, kind, reason, quote, registrationId, cohortName }) {
  const money = n => 'UGX ' + Number(n || 0).toLocaleString('en-GB');
  const isD = kind === 'defer';
  const bodyHtml = `
    <p style="margin:0 0 16px;"><strong style="color:#1A1A1A">${asgEsc(founderName)}</strong> has asked to ${isD ? 'postpone' : 'withdraw'}${cohortName ? ` from ${asgEsc(cohortName)}` : ''}.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;margin:0 0 18px;">
      <tr><td style="padding:14px 18px;font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.65;color:#2A2826;white-space:pre-wrap;">${asgEsc(String(reason || '').slice(0, 800))}</td></tr>
    </table>
    <p style="margin:0 0 8px;font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;">
      Booking #${registrationId} &middot; ${asgEsc(founderEmail)}<br>
      ${isD
        ? `${money(quote.credit_amount)} carries forward, nothing charged.`
        : `Refund would be <strong>${money(quote.refund_amount)}</strong> (${money(quote.amount_paid)} paid &minus; ${money(quote.deposit_retained)} deposit &minus; ${money(quote.delivery_charge)} for ${quote.sessions_delivered}/${quote.sessions_total} delivered).`}
    </p>
    <p style="margin:0 0 22px;font-family:'Inter',Arial,sans-serif;font-size:13px;color:#5A564F;">Approving moves their seat, withdraws the calendar invites and pauses any instalment plan.</p>
    ${emailBtn('Open the queue', ASG_BASE + '/login-admin.html')}`;
  const html = fsEmailShell({
    accent: isD ? '#C9923A' : '#9A3E16',
    eyebrow: isD ? 'Postponement requested' : 'Withdrawal requested', eyebrowColor: '#9A3E16',
    headline: 'A founder needs a decision.',
    bodyHtml,
  });
  return sendEmail({ to: ADMIN_EMAIL, subject: asgSubject(`${founderName} — ${ENR_LABEL[kind]} request`), html, template: 'enrollment_request_staff' });
}

async function sendEnrollmentDecision({ to, firstName, kind, decision, quote, newCohortName, note }) {
  const money = n => 'UGX ' + Number(n || 0).toLocaleString('en-GB');
  const isD = kind === 'defer';
  ...
  if (!approved) {
    bodyHtml = `
      <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
      <p style="margin:0 0 22px;">We haven't been able to approve your ${ENR_LABEL[kind]} request as it stands.</p>
      ${note ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;margin:0 0 22px;"><tr><td style="padding:14px 18px;font-family:'Inter',Arial,sans-serif;font-size:14px;line-height:1.65;color:#2A2826;white-space:pre-wrap;">${asgEsc(note)}</td></tr></table>` : ''}
      <p style="margin:0 0 22px;">Reply to this email and we'll work it out with you — this isn't the end of the conversation.</p>`;
  } else if (isD) {
    bodyHtml = `
      <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
      <p style="margin:0 0 22px;">Done. You're moved to <strong style="color:#1A1A1A">${asgEsc(newCohortName || 'the next cohort')}</strong>, and the ${money(quote.credit_amount)} you've paid carries across in full.</p>
      <p style="margin:0 0 22px;">Your calendar invites for the current cohort have been withdrawn, so they'll disappear from your diary. You'll get fresh ones before the new cohort starts, and you'll begin at Week 1 with the rest of that group.</p>
      <p style="margin:0 0 22px;">Look after yourself. We'll see you in the new year.</p>
      ${emailBtn('Your dashboard', ASG_BASE + '/login-founder.html')}`;
  } else {
    bodyHtml = `
      <p style="margin:0 0 16px;">Hi ${asgEsc(firstName) || 'there'},</p>
      <p style="margin:0 0 22px;">Your withdrawal is confirmed, and we're sorry to see you go.</p>
      ${quote.refund_amount > 0
        ? `<p style="margin:0 0 22px;">A refund of <strong style="color:#1A1A1A">${money(quote.refund_amount)}</strong> is with our finance team and will be sent to the number or account you paid from. If you haven't seen it within seven working days, reply to this email.</p>`
        : `<p style="margin:0 0 22px;">There's no refund due on this booking — the deposit is non-refundable and covers the sessions already delivered.</p>`}
      <p style="margin:0 0 22px;">If your circumstances change, you'd be welcome back on any future cohort. Just say the word.</p>`;
  }
  const html = fsEmailShell({
    accent: approved && isD ? '#3D4A2E' : '#9A3E16',
    eyebrow: approved ? (isD ? 'Postponement confirmed' : 'Withdrawal confirmed') : 'About your request',
    eyebrowColor: '#9A3E16',
    headline: approved ? (isD ? 'Your place is held.' : 'All settled.') : 'Let’s talk.',
    bodyHtml,
  });
  return sendEmail({
    to, subject: asgSubject(approved ? `Your ${ENR_LABEL[kind]} is confirmed` : `Your ${ENR_LABEL[kind]} request`),
    html, template: 'enrollment_decision',
  });
}
...
// ── Corporate advisory: client portal invite ────────────────────────────────
async function sendCorporateClientInvite({ to, name, clientName, engagementTitle, actionUrl }) {
  const bodyHtml = `
    <p style="margin:0 0 14px">Hi ${name || 'there'},</p>
    <p style="margin:0 0 14px">You've been invited to the advisory engagement for <strong>${clientName || 'your company'}</strong> with The Founder's Sprint${engagementTitle ? ` — <em>${engagementTitle}</em>` : ''}.</p>
    <p style="margin:0 0 18px">Set your password to open your secure client portal. Your first step is a mutual non-disclosure agreement — you can read it in the portal, download it for your legal team to review, or sign it right away. Your due-diligence document vault unlocks the moment the NDA is signed.</p>
    ${emailBtn('Open your client portal', actionUrl)}
    <p style="margin:20px 0 0;font-size:13px;color:#8b8578">Everything you share in the portal is confidential and visible only to your Founder's Sprint advisory team.</p>`;
  const html = fsEmailShell({ eyebrow: 'Advisory engagement', headline: 'Your client portal is ready', bodyHtml });
  return sendEmail({ to, subject: "You've been invited to your Founder's Sprint advisory portal", html, template: 'corporate_client_invite' });
}


// ── Session reminder, 72h before a scheduled session ─────────────────────────
// Bulk-generating a cohort creates the calendar events SILENTLY, so this is the
// notification founders and coaches actually receive: one email per session,
// three days out, carrying the Meet link. Sent to founders and the coach alike.
/**
 * The reminder headline used to be the string 'Coming up in three days', hardcoded,
 * because the drip normally fires at T-72h. It doesn't always: a session generated
 * inside the window, or one whose reminder is re-sent, goes out far later — so a
 * coach was told a class was "in three days" directly above a line saying it was in
 * eleven hours. Derive both from the actual time.
 *
 * Day boundaries are counted in Africa/Kampala, not in hours, so a session at 09:00
 * tomorrow reads "tomorrow" whether it is 15 hours away or 23.
 */
const NUM_WORD = ['zero','one','two','three','four','five','six','seven'];
function eatDayKey(d) {
  return new Date(d).toLocaleDateString('en-CA', { timeZone: 'Africa/Kampala' }); // YYYY-MM-DD
}
function sessionTiming(scheduledAt, hoursAway) {
  const h = Math.max(0, Number(hoursAway) || 0);
  const dayDiff = Math.round(
    (Date.parse(eatDayKey(scheduledAt)) - Date.parse(eatDayKey(new Date()))) / 86400000);

  if (h < 1)   return { headline: 'Starting shortly',          phrase: 'in under an hour' };
  if (h < 2)   return { headline: 'Coming up within the hour', phrase: 'in about an hour' };
  if (dayDiff <= 0) return { headline: 'Later today',          phrase: `in about ${Math.round(h)} hours` };
  if (dayDiff === 1) return { headline: 'Coming up tomorrow',
                              phrase: h < 24 ? `in about ${Math.round(h)} hours` : 'tomorrow' };
  const word = NUM_WORD[dayDiff] || String(dayDiff);
  return { headline: `Coming up in ${word} days`, phrase: `in ${word} days` };
}

async function sendSessionReminder({ to, name, session, meetLink, isCoach = false, hoursAway = 72 }) {
  const BASE = 'https://founderssprint.co';
  const timing = sessionTiming(session.scheduled_at, hoursAway);
  const when = new Date(session.scheduled_at).toLocaleString('en-GB', {
    weekday: 'long', day: 'numeric', month: 'long',
    hour: '2-digit', minute: '2-digit', hour12: false, timeZone: 'Africa/Kampala',
  });
  const mins = session.duration_minutes || 120;
  const bodyHtml = `
    <p style="margin:0 0 20px;">Hi ${name || 'there'},</p>
    <p style="margin:0 0 20px;">${isCoach ? 'You\'re leading' : 'You have'} <strong style="color:#1A1A1A">${session.title}</strong> ${timing.phrase}.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#C9923A;">&nbsp;</td></tr>
      <tr><td style="padding:20px 22px;text-align:center;">
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:26px;color:#1A1A1A;line-height:1.2;">${when} EAT</div>
        <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8A6D1F;margin-top:8px;">${mins} minutes</div>
      </td></tr>
    </table>
    ${meetLink ? emailBtn('Join on Google Meet', meetLink) : '<p style="margin:0 0 12px;">The meeting link will follow shortly.</p>'}
    <p style="margin:16px 0 0;font-family:'Inter',Arial,sans-serif;font-size:13px;line-height:1.6;color:#5A564F;">The invite is attached — open it to add this session to your calendar. If the time no longer works, reply to this email and we\'ll sort it out.</p>`;
  const html = fsEmailShell({ eyebrow: 'Session reminder', headline: timing.headline, bodyHtml });

  // Attach the invite as .ics so it works on ANY calendar — Apple, Outlook,
  // Thunderbird — not just Google. The Google event is created silently, which
  // only reaches Google Calendar users; this covers everyone else at the same
  // cadence, without a second email. UID is stable per session, so re-issuing
  // updates the entry rather than duplicating it.
  let attachments = [];
  try {
    const ics = buildSessionIcs({
      uid: 'session-' + session.id + '@founderssprint.co',
      title: session.title,
      description: session.description || session.title,
      start: session.scheduled_at,
      durationMinutes: mins,
      meetLink,
      organizerEmail: process.env.GOOGLE_CALENDAR_DELEGATE || FROM_EMAIL,
      organizerName: "The Founder's Sprint",
      attendees: [{ email: to, name }],
    });
    attachments = [{
      filename: 'founders-sprint-session.ics',
      content: Buffer.from(ics, 'utf8'),
      content_type: 'text/calendar; charset=utf-8; method=REQUEST',
    }];
  } catch (e) {
    // An invite that won't build must never block the reminder itself.
    console.error('[Email] ICS build failed for session', session.id, e.message);
  }

  return sendEmail({ to, subject: session.title + ' — ' + when + ' EAT', html,
                     template: 'session_reminder', attachments });
}


// ── Cohort programme schedule — the "here is your whole coaching plan" email ──
// A founder who has just paid needs to see the shape of the programme, not learn
// it one 72h reminder at a time. Sent once on confirmation; the drip then handles
// the per-session nudges. Rows are grouped by week so a 25-session series reads
// as five weeks rather than a wall of dates.
async function sendCohortSchedule(reg, cohort, sessions) {
  const BASE = 'https://founderssprint.co';
  ...
  const byWeek = {};
  (sessions || []).forEach(s => {
    const m = /Week (\d+)/.exec(s.title || '');
    const w = m ? m[1] : '1';
    (byWeek[w] = byWeek[w] || []).push(s);
  });

  const weeks = Object.keys(byWeek).sort((a, b) => a - b).map(w => {
    const rows = byWeek[w].sort((a, b) => new Date(a.scheduled_at) - new Date(b.scheduled_at)).map(s => {
      const topic = String(s.title || '').split('·').pop().replace(/—.*$/, '').trim();
      return `<tr>
        <td style="padding:7px 10px 7px 0;font-size:13px;color:#1A1A1A;white-space:nowrap">${fmtWhen(s.scheduled_at)}</td>
        <td style="padding:7px 0;font-size:13px;color:#5A564F">${topic}</td>
      </tr>`;
    }).join('');
    return `<div style="margin:0 0 16px">
      <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8A6D1F;margin-bottom:4px">Week ${w}</div>
  ...
  const bodyHtml = `
    <p style="margin:0 0 20px;">Hi ${reg.first_name},</p>
    <p style="margin:0 0 20px;">You're confirmed for <strong style="color:#1A1A1A">${cohort ? cohort.name : 'the cohort'}</strong>. Here is your full programme — five coaches, five disciplines, every session already in the calendar.</p>
    <p style="margin:0 0 18px;">All times are <strong style="color:#1A1A1A">East Africa Time</strong>. You'll get a reminder with the joining link three days before each session, so there's nothing to keep track of.</p>
    ${weeks}
    ${emailBtn('Open your dashboard', BASE + '/login-founder.html')}
    <p style="margin:16px 0 0;font-family:'Inter',Arial,sans-serif;font-size:13px;line-height:1.6;color:#5A564F;">If any of these times clash with something you can't move, reply to this email and we'll work around it.</p>`;

  const html = fsEmailShell({ eyebrow: 'Your programme', headline: 'Your coaching schedule', bodyHtml });
  return sendEmail({
    to: reg.email,
    subject: (cohort ? cohort.name : 'Your cohort') + ' — your full coaching schedule',
    html, registrationId: reg.id, template: 'cohort_schedule',
  });
}
...
// ── Monthly check-in nudge (Phase 0 longitudinal spine) ───────────────────────
async function sendMonthlyNudge(founder, monthLabel, dashUrl) {
  const first = ((founder && founder.first_name) || 'there').toString().trim() || 'there';
  const bodyHtml = `
    <p style="margin:0 0 16px;">Hi ${first},</p>
    <p style="margin:0 0 18px;">Ninety seconds is the whole ask, and it's the most valuable habit you can build here: log this month's numbers for your venture — revenue, cash on hand, customers and team size.</p>
    <p style="margin:0 0 22px;">A steady month-on-month record is the strongest signal you can show an investor or lender. They trust a founder who reports consistently long before they trust any single figure. Your check-ins stay private to you, your coaches and our team.</p>
    ${emailBtn('Log your ' + monthLabel + ' check-in', dashUrl)}
    <p style="margin:24px 0 0;font-size:13px;color:#5A564F;">You're getting this because you have a venture on The Founder's Sprint. One short check-in a month — that's it.</p>
  `;
  const html = fsEmailShell({
    accent:       '#3D4A2E',
    eyebrow:      'Monthly check-in',
    eyebrowBg:    'rgba(61,74,46,0.15)',
    eyebrowColor: '#2C3720',
    headline:     `Your ${monthLabel} check-in`,
    bodyHtml,
  });
  return sendEmail({
    to:             founder.email,
    subject:        `Your ${monthLabel} check-in · The Founder's Sprint`,
    html,
    template:       'monthly_nudge',
    registrationId: null,
  });
}


// ── Soft-hold journey emails (72h seat hold pending deposit) ─────────────────
function fmtHoldDeadline(ts) {
  try {
    return new Date(ts).toLocaleString('en-GB', {
      weekday: 'short', day: 'numeric', month: 'short',
      hour: '2-digit', minute: '2-digit', timeZone: 'Africa/Kampala'
    }) + ' EAT';
  } catch (e) { return 'in 72 hours'; }
}

// STATE: HELD — deposit initiated, seat reserved for 72h, awaiting PIN.
async function sendHoldStarted(reg, cohort) {
  const BASE = 'https://founderssprint.co';
  const what = cohort ? cohort.name : (reg.track === 'cohort' ? 'your cohort' : 'your session');
  const deadline = reg.hold_expires_at ? fmtHoldDeadline(reg.hold_expires_at) : 'in 72 hours';
  const bodyHtml = `
    <p style="margin:0 0 20px;">Hi ${reg.first_name},</p>
    <p style="margin:0 0 20px;">Your seat for <strong style="color:#1A1A1A">${what}</strong> is <strong style="color:#1A1A1A">reserved</strong> — we're holding it while you complete the 10% deposit.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:#C9923A;">&nbsp;</td></tr>
      <tr><td style="padding:20px 22px;text-align:center;">
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:38px;color:#1A1A1A;line-height:1;">${fmt(reg.deposit_amount)}</div>
        <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8A6D1F;margin-top:8px;">Deposit to secure your seat</div>
      </td></tr>
    </table>
    <p style="margin:0 0 12px;">We've sent a payment prompt to your phone — <strong style="color:#1A1A1A">enter your PIN</strong> to confirm. Your seat is held until <strong style="color:#1A1A1A">${deadline}</strong>; after that it's released to the next founder.</p>
    ${emailBtn('Complete your deposit', BASE + '/login-founder.html')}
    <p style="margin:16px 0 0;font-family:'Inter',Arial,sans-serif;font-size:13px;line-height:1.6;color:#5A564F;">Didn't get the prompt? Open your dashboard to pay your deposit, or just reply here and we'll help.</p>
  `;
  const html = fsEmailShell({ accent: '#C9923A', eyebrow: 'Seat on hold', eyebrowColor: '#8A6D1F', eyebrowBg: 'rgba(201,146,58,0.18)', headline: `We're holding your seat, ${reg.first_name}.`, bodyHtml });
  return sendEmail({ to: reg.email, subject: `Your seat is on hold — complete your deposit · ${what}`, html, template: 'hold_started', registrationId: reg.id });
}

// STATE: HELD, expiring — reminder touches (1 ~24h, 2 ~3h before release).
async function sendHoldReminder(reg, cohort, touch) {
  const BASE = 'https://founderssprint.co';
  const what = cohort ? cohort.name : (reg.track === 'cohort' ? 'your cohort' : 'your session');
  const deadline = reg.hold_expires_at ? fmtHoldDeadline(reg.hold_expires_at) : 'soon';
  const final = Number(touch) >= 2;
  const bodyHtml = `
    <p style="margin:0 0 20px;">Hi ${reg.first_name},</p>
    <p style="margin:0 0 20px;">${final ? 'This is a final reminder —' : 'A quick reminder —'} your held seat for <strong style="color:#1A1A1A">${what}</strong> is released ${final ? 'very soon' : 'shortly'} unless the 10% deposit lands.</p>
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="fspd" style="background:#E6DCC7;border-radius:0 0 20px 0;margin:0 0 22px;">
      <tr><td style="height:3px;line-height:3px;font-size:0;background:${final ? '#9A3E16' : '#C9923A'};">&nbsp;</td></tr>
      <tr><td style="padding:20px 22px;text-align:center;">
        <div style="font-family:'Cormorant Garamond',Georgia,serif;font-size:34px;color:#1A1A1A;line-height:1;">${fmt(reg.deposit_amount)}</div>
        <div style="font-family:'Josefin Sans',Arial,sans-serif;font-size:10px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#8A6D1F;margin-top:8px;">Seat held until ${deadline}</div>
      </td></tr>
    </table>
    ${emailBtn('Pay my deposit now', BASE + '/login-founder.html', final ? '#9A3E16' : '#C8531F')}
  `;
  const html = fsEmailShell({ accent: final ? '#9A3E16' : '#C9923A', eyebrow: final ? 'Final reminder' : 'Seat expiring', eyebrowColor: '#9A3E16', headline: final ? 'Last chance to keep your seat.' : `Your seat is expiring, ${reg.first_name}.`, bodyHtml });
  return sendEmail({ to: reg.email, subject: `${final ? 'Final reminder' : 'Reminder'}: your seat is on hold · ${what}`, html, template: `hold_reminder_${touch}`, registrationId: reg.id });
}

// STATE: EXPIRED/lapsed — hold released, kept as a re-bookable Interest lead.
async function sendHoldLapsed(reg, cohort) {
  const BASE = 'https://founderssprint.co';
  const what = cohort ? cohort.name : (reg.track === 'cohort' ? 'your cohort' : 'your session');
  const bodyHtml = `
    <p style="margin:0 0 20px;">Hi ${reg.first_name},</p>
    <p style="margin:0 0 20px;">Your held seat for <strong style="color:#1A1A1A">${what}</strong> was released because the deposit didn't come through in time — <strong style="color:#1A1A1A">no charge was made</strong>. Good news: you can pick it straight back up while places last.</p>
    ${emailBtn('Rebook my seat', BASE + '/book/')}
    <p style="margin:16px 0 0;font-family:'Inter',Arial,sans-serif;font-size:13px;line-height:1.6;color:#5A564F;">Need a hand or a different date? Just reply — we'll sort it.</p>
  `;
  const html = fsEmailShell({ accent: '#777770', eyebrow: 'Seat released', eyebrowColor: '#5A564F', eyebrowBg: 'rgba(119,119,112,0.16)', headline: 'Your hold has expired — but you can rebook.', bodyHtml });
  return sendEmail({ to: reg.email, subject: `Your seat was released — rebook when ready · ${what}`, html, template: 'hold_lapsed', registrationId: reg.id });
}

module.exports = {
  sendMonthlyNudge,
  sendStaffInvite,
  sendCorporateClientInvite,
  ...
  sendRefundOpsAlert,
  sendExpressBookingRequest,
  sendExpressBookingAdmin,
  sendExpressConfirmed,
  sendAdminReport,
  ...
  // Postponement & withdrawal
  sendEnrollmentRequestFounder,
  sendEnrollmentRequestStaff,
  sendEnrollmentDecision,
};
