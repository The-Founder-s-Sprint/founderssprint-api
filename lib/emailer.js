...
async function sendNewRegistrationAdmin(reg, cohort, isPreview = false) {
  ...
  // Which 1-on-1 specialty/specialties they chose (L3 slugs → readable labels).
  const specs = Array.isArray(reg.enrolled_specialties) ? reg.enrolled_specialties.filter(Boolean) : [];
  const specLine = specs.length
    ? specs.map(s => String(s).split('-').map(w => w ? w.charAt(0).toUpperCase() + w.slice(1) : w).join(' ')).join(', ')
    : null;
  ...
  if (specLine) rows.push([specs.length > 1 ? 'Sessions' : 'Session', specLine]);
